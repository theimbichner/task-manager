...
def test_date_time_start_init():
    date_time = task_data.DateTime(time_start)

    assert date_time.start == time_start
    assert date_time.end == date_time.start

def test_date_time_start_end_init():
    date_time = task_data.DateTime(time_start, time_end)

    assert date_time.start == time_start
    assert date_time.end == time_end

def test_date_time_eq():
    date_time_1 = task_data.DateTime(time_start, time_end)
    date_time_2 = task_data.DateTime(time_start, time_end)

    assert date_time_1 == date_time_2
    assert not (date_time_1 != date_time_2)

neq_date_times = [
    task_data.DateTime(time_start - 1, time_end),
    task_data.DateTime(time_start, time_end + 1),
    task_data.DateTime(time_start - 1, time_end + 1),
    'datetime',
    None
]

@pytest.mark.parametrize('comparison', neq_date_times)
def test_date_time_neq(comparison):
    date_time = task_data.DateTime(time_start, time_end)
...
def test_date_time_to_from_dict():
    ...
    new_date_time = task_data.DateTime.from_dict(data)
